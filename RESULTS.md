...
Same settings as the batch sweep (BF16, eager, prompt 256, 128 new tokens, 2 warmup + 5 measured). Two passes per configuration (descending, ascending), agreeing to within about 1.5% (max 1.52%) at every valid cell. 135M was re-run in the same session and matched the original sweep within 1.1% up to batch 128 (4.5% less time at 256: 12684 vs 13286 ms).
...
- Batch-1 step time: 18.0 / 19.2 / 10.8 ms (135M / 360M / 1.7B), i.e. 1.7B takes 40% less time per step than 135M (~1.67× the speed) and 360M ~6% more. Not proportional to parameters; see the decode-step profile below for the measured breakdown.
...
**Method.** `scripts/profile_decode.py`: BF16, eager, SDPA attention, synthetic prompt of 256 tokens, manual greedy decode loop over `DynamicCache` (or `StaticCache` for the static cells) that mirrors what `generate()` does per step. Per cell: 8 warmup steps, then 32 timed steps synchronized after every step (wall), 32 timed steps without per-step synchronization (host dispatch), a count of synchronizing CUDA calls in one step, and 10 steps (after 2 skipped) under `torch.profiler` with CPU + CUDA (CUPTI) activity. GPU busy = union of kernel/memcpy/memset intervals per step; idle fraction = 1 − busy / unprofiled wall; weight bandwidth = BF16 weight bytes / GPU busy time (rated 672 GB/s). `generate()` is also timed end to end (1 warmup + 2 iterations, 128 new tokens). Two passes, the second in reverse cell order; values below are the mean of the two passes, with ± half the range between them for wall, dispatch and GPU busy. `scripts/probe_sdpa_gqa.py` checks which SDPA backends accept the decode shapes and reruns the GQA models with Transformers' GQA fast path disabled.

| Model | Cache | Batch | Wall ms | Dispatch ms | GPU busy ms | GPU idle | Kernels/step | Weight BW (b1) |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| 135M | dynamic | 1 | 21.01 ± 1.10* | 24.53 ± 4.71* | 2.73 ± 0.01 | 87% | 1680 | 98 GB/s (15%) |
| 135M | dynamic | 32 | 19.88 ± 0.26 | 19.90 ± 0.03 | 8.50 ± 0.01 | 57% | 1680 | |
| 135M | dynamic | 128 | 44.14 ± 0.03 | 43.35 ± 0.24 | 36.56 ± 0.00 | 17% | 1680 | |
| 360M | dynamic | 1 | 24.53 ± 3.26* | 23.82 ± 2.34* | 4.17 ± 0.00 | 83% | 1790 | 173 GB/s (26%) |
| 360M | dynamic | 16 | 21.03 ± 0.00 | 20.98 ± 0.01 | 7.92 ± 0.03 | 62% | 1790 | |
| 360M | dynamic | 64 | 41.09 ± 0.01 | 40.16 ± 0.03 | 33.19 ± 0.04 | 19% | 1822 | |
| 1.7B | dynamic | 1 | 11.89 ± 0.02 | 11.75 ± 0.04 | 7.24 ± 0.00 | 39% | 990 | 473 GB/s (70%) |
| 1.7B | dynamic | 16 | 14.87 ± 0.01 | 14.70 ± 0.05 | 11.85 ± 0.01 | 20% | 990 | |
| 1.7B | dynamic | 32 | 19.87 ± 0.03 | 19.51 ± 0.09 | 16.55 ± 0.01 | 17% | 1014 | |
| 1.7B | static | 1 | 13.76 ± 0.09 | 13.40 ± 0.03 | 7.52 ± 0.00 | 45% | 1144 | 455 GB/s (68%) |
| 1.7B | static | 32 | 17.20 ± 0.01 | 16.46 ± 0.15 | 13.21 ± 0.01 | 23% | 1170 | |

All values are means of two passes; ± is half the range between the passes. \* Anomalous first pass for these cells: wall 22.1 / 19.9 ms (135M) and 27.8 / 21.3 ms (360M); dispatch 29.2 / 19.8 and 26.2 / 21.5 ms. The second pass matches the probe's default-path runs (19.9–20.0 and 21.4–21.6 ms). Kernel counts are identical across passes and GPU busy times agree within 1%.
...
- **Below the knee, steps are host-bound.** At batch 1 the GPU is idle 87% / 83% / 39% of the step. Host dispatch time / kernels = 11.7–12.0 µs in every below-knee cell (excluding the starred cells' first passes: 17.4 and 14.6 µs), and host time outside sync waits (under the profiler) varies by < 4% across batch sizes for each model with the dynamic cache (cell means; for 135M b1 second pass only, since its first pass gave 45.3 vs 27.2 ms; the 360M b1 host time agreed across passes, 29.2 / 29.4 ms).
...
- **Why 1.7B is faster per step.** After the counterfactual, step time / layers = 0.53 / 0.53 / 0.50 ms (135M / 360M / 1.7B). 1.7B vs 135M at batch 1 (11.9 vs 20.0 ms; the 20.0 is the probe's default-path run, not the two-pass mean in the table above) splits roughly equally between the GQA fallback (4.1 ms) and six fewer layers (4.0 ms); vs 360M, 4.4 and 5.2 ms.
...
- **Static vs dynamic cache (1.7B).** At batch 32, 29% of dynamic-cache GPU kernel time is `DynamicCache` `torch.cat` copies; the static cache avoids them (busy 13.2 vs 16.5 ms; step 17.2 vs 19.9 ms), consistent with its 15% lower latency at batch 32 in the model-size sweep (2751 vs 3232 ms, ~1.17× the throughput). At batch 1 the static cache is slower (13.8 vs 11.9 ms) mainly from 154 more kernels per step; attention over the full preallocated length adds only ~0.2 ms of GPU time.
- **135M crossover.** Linear interpolation of GPU busy time between b32 (8.5 ms) and b128 (36.6 ms) reaches the ~20 ms host time (135M b32 step, 19.9 ms) near b ≈ 70 at KV length 280; only three points, and the sweep's latency already rises by batch 64.
- **Profiler overhead.** Profiled step time / unprofiled wall = 1.34–1.40× below the knee (excluding the starred cells' first passes: 2.05× for 135M b1 and 1.06× for 360M b1), ≤ 1.15× above it; idle fractions use the unprofiled wall.

Session offset: this profile ran in a later session than the model-size sweep. `generate()` (same prompt, 128 new tokens) took 11.6–13.2% longer than in the model-size sweep in every host-bound cell but only 0.7–1.2% longer in the five GPU-bound cells (135M b128, 360M b64, 1.7B dynamic b16/b32, 1.7B static b32), i.e. the session difference is host-side. Batch 1 `generate()` end to end: 2603 / 2782 / 1561 ms (20.3 / 21.7 / 12.2 ms per token) vs 2308 / 2457 / 1379 ms (18.0 / 19.2 / 10.8) in the sweep; model ratios are unchanged (360M/135M 1.07, 1.7B/135M 0.60). Compare absolute profile times with each other, not with the sweep. Windows only: no Linux comparison, so WDDM effects on host cost per kernel are not isolated.
...
Superseded earlier run: an earlier run of the same script (same cells and settings) was superseded because a background RGB-lighting application was using CPU during it. Its kernel counts were identical and its GPU busy times agreed within 1.3% (largest difference 135M b128, 36.08 vs 36.56 ms), but `generate()` in the host-bound cells took 16.9–19.3% longer than in the model-size sweep (vs 11.6–13.2% in the reported run); its GPU-bound cells were 0.6–1.2% longer, as in the reported run. Its `summary.json`, `raw.json` and `sdpa_gqa_probe.json` are in `results/profile_decode_run1/` (their `out_dir` field still reads `results\profile_decode`, the directory name at run time); its traces are not committed.

### Smoke baseline (earlier)

SmolLM-135M fp16, batch 1, prompt 64, gen 32: E2E mean **624.5 ms**, ~52 tok/s, ~268 MiB peak.

Note: `results/smoke_gpu_summary.json` was edited once after the run (commit db0a956): the machine's hostname was removed from `env` for privacy, and an `os` field ("Windows 11 (build 26200)") was added because the recorded `platform` string, left unchanged, reports Windows 10 for this Windows 11 build. No measured values were changed.
